...
describe("isMenuActive", () => {
  ...
  it("should identify extension (dashboard extensions from apps) menu items as active when their URL matches", () => {
...
describe("hasActiveDescendant", () => {
  it("should identify item groups with an active child", () => {
    const section: SidebarMenuItem = {
      id: "catalog",
      label: "Catalog",
      url: "/products",
      type: "itemGroup",
    ...
    expect(hasActiveDescendant("/categories", section)).toBe(true);
  });

  it("should ignore the group's own landing page when looking for active descendants", () => {
    const section: SidebarMenuItem = {
      id: "catalog",
      label: "Catalog",
      url: "/products",
    ...
    expect(hasActiveDescendant("/products", section)).toBe(false);
  });
});

describe("isCurrentMenuItem", () => {
  it("should identify the current page item separately from active ancestors", () => {
    const section: SidebarMenuItem = {
      id: "catalog",
      label: "Catalog",
      url: "/products",
    ...
    expect(isCurrentMenuItem("/products", section)).toBe(true);
    expect(hasActiveDescendant("/categories", section)).toBe(true);
    expect(isCurrentMenuItem("/categories", section)).toBe(false);
  });
});
